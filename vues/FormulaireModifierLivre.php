<?php
    ...
    if(isset($_POST["titre"])) $titre = $_POST["titre"]; else $titre = $livre["titre"];

    if(isset($_POST["description"])) $description = $_POST["description"]; else $description = $livre["description"];

    if(isset($_POST["ISBN"])) $ISBN = $_POST["ISBN"]; else $ISBN = $livre["ISBN"];

    if(isset($_POST["editeur"])) $id_editeur = $_POST["editeur"]; else $id_editeur = $livre["id_editeur"];

    if(isset($_POST["auteur"])){

        $auteurs_selectionne = $_POST["auteur"];

     } else {

        $auteurs_selectionne = [];
        while($auteur_mysqli = mysqli_fetch_assoc($donnees["auteurs_selectionne"])) {
            $auteurs_selectionne[] = $auteur_mysqli["auteur_id"];
        }
...
<main>
    ...
    <form action="index.php?commande=ModifierLivre&id=<?=$livre["id"]?>#erreur" method="post" class="formulaire" enctype="multipart/form-data">
        <div>
